...
C-LIVE-ARTIFACT confirmed at payload level that the pinned Codex subscription route omits a provider token cap. Live probes injecting the standard Responses field at 128, 1,024, 2,048, and 4,096 were rejected by that backend. The adapter therefore records `maxOutputTokens: 4_096` as requested but unsupported, with no forwarded, adjusted, or provider-verified value. Observed output and reasoning usage remain evidence only and are not treated as enforcement. The two-round limit, 180-second debate timeout, and disposal are the live smoke's hard bounds. A true total-output-token cap, including hidden reasoning, requires switching to a provider route that supports it.
