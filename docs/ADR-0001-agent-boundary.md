...
Task B-LIVE-DEBATE verified four ordered turns across two separately instantiated role agents, with reset/replay context selection and the `[5, 5, 1, 1]` creativity sequence. The provider again omitted `responseModel`. It also reported output usage above the requested `maxOutputTokens: 128`; later inspection established that Pi 0.80.10's Codex request builder accepted `maxTokens` but omitted `max_output_tokens` from the emitted payload.

C-LIVE-ARTIFACT confirmed at payload level that the pinned Codex subscription route omits a provider token cap, and a probe injecting the standard Responses field was rejected by that backend. The adapter therefore reports `maxOutputTokens` as unsupported rather than forwarded. It separately enforces the requested numeric value as a conservative client ceiling on observable UTF-8 output bytes and aborts before forwarding an over-limit delta; because every token contains at least one byte, this bounds observable output tokens without claiming provider verification. Hidden reasoning is not stream-observable, so the live smoke also requires final reported total output usage at or below the selected 4,096 per-turn limit. The larger limit leaves room for high-effort hidden reasoning; 128, 1,024, and 2,048 provider-field probes produced no visible answer. Timeout and disposal remain additional live-test bounds until domain budgets exist.
